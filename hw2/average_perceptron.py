...
class average_perceptron():
	...
	def __init__(self, n, learning_rate, epochs):
		...
		self.e = epochs
		self.perceptron = np.array([np.random.rand(n) for i in range(10)])
		self.sum_perceptron = np.array([np.zeros(n) for i in range(10)])
		...
		for epoch in range(self.e):				# epochs
			for digit in range(10):				# perceptrons 0-9 for diff class
				for i, x in enumerate(data):
					...
					if score > 0:							# predict = True
						if digit == label[i]:	continue	# classified correctly
						else:					self.perceptron[digit] -= self.r*x
					else:									# predict = False
						if digit != label[i]:	continue	# classified correctly
						else:					self.perceptron[digit] += self.r*x
					self.sum_perceptron[digit] += self.perceptron[digit]
	...
	def classify(self, data):
		...
		for row in data:
			...
			for digit in range(10):
				score = np.dot(row, self.sum_perceptron[digit])
...
def main():
	...
	# start training
	model = average_perceptron(len(train_data[0]), learning_rate, epochs)
